/* ═══════════════════════════════════════════════════════════════
   OpenSim — core/lycoming-processor.js
   Lycoming IO-360 — 4-cylinder, 180hp — C172 engine
   Same physical model as DB 605, adapted for 4 cylinders.
   4-stroke: 2 firings per revolution at 0° and 180°.
   ═══════════════════════════════════════════════════════════════ */

class LycomingProcessor extends AudioWorkletProcessor {
  constructor() {
    super();
    ...
    /* 4-cylinder 4-stroke: 2 firings per revolution at 0° and 180° */
    this.crankAngle    = 0;
    this.firingAngles  = [0, 180].map(a => (a + (Math.random() - 0.5) * 10) % 360);
    this.cylinderGains = Array.from({ length: 4 }, () => 0.5 + Math.random() * 1.0);

    this.transient  = 0;
    this.transDecay = Math.exp(-300 / sampleRate);   // ~15ms

    this.body       = 0;
    this.bodyDecay  = Math.exp(-55  / sampleRate);

    /* Resonator — Lycoming exhaust ~95 Hz, less resonant than DB 605 */
    const f0    = 95;
    const r     = 0.94;
  ...
  _updateBodyDecay(rpm) {
    /* 4-cyl 4-stroke: 2 firings per revolution */
    const firingInterval = sampleRate * 30 / rpm;  // 60/(rpm*2) * sr
    ...
    for (let i = 0; i < out.length; i++) {
      ...
      for (let c = 0; c < this.firingAngles.length; c++) {
        if (_crossed(prev, this.crankAngle, this.firingAngles[c])) {
          const amp       = this.cylinderGains[c % 4];
          this.transient += amp;
          this.body      += amp * 0.7;
        }
      }
...
registerProcessor('lycoming-processor', LycomingProcessor);
